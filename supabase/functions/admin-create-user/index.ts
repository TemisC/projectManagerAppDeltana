...
// profiles.role, so Gerencia can manage the team from inside the app
...
const VALID_ROLES = ["gerencia", "gestor", "colaborador"];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: callerProfile, error: profileError } = await callerClient
      ...
      .eq("id", caller.id)
      .single();
    if (profileError || callerProfile?.role !== "gerencia") {
      return json({ error: "Solo Gerencia puede crear usuarios." }, 403);
    }
